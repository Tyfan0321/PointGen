...
class DepthwiseKPConv(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, radius, sigma, bias=False):
        """Initialize parameters for KPConv.
        ...
        Args:
             in_channels: dimension of input features.
             out_channels: dimension of output features.
             kernel_size: Number(s) of kernel points.
             radius: radius used for kernel point init.
             sigma: influence radius of each kernel point.
             bias: use bias or not (default: False)
        """
        super(DepthwiseKPConv, self).__init__()
        ...
        if isinstance(self.kernel_size, int):
            self.weights = nn.Parameter(torch.zeros(self.kernel_size, in_channels, out_channels/in_channels))
            kernel_points = load_kernels(self.radius, self.kernel_size, 3, fixed='center')  # (N, 3)
        else:
            num_kernels = sum(self.kernel_size) - len(self.kernel_size) + 1
            self.weights = nn.Parameter(torch.zeros(num_kernels, in_channels, out_channels/in_channels))
            kernel_points = np.concatenate([np.zeros([1, 3]), [
                load_kernels(1., ks, 3, fixed='center', scale=0.)[1:] * (self.radius/0.66*(l+1)/(len(self.kernel_size)+1))
                for l,ks in enumerate(self.kernel_size)]
            ], axis=0)  # (N, 3)
    ...
    def forward(self, s_feats, q_points, s_points, neighbor_indices):
        ...
        # apply convolutional weights
        weighted_feats = weighted_feats.unsqueeze(-1).expand(*weighted_feats.shape, self.out_channels/self.in_channels)
        kernel_outputs = (weighted_feats * self.weights.unsqueeze(dim=0))  # (M, K, C) -> (M, K, C, C_out/C)
        output_feats = torch.sum(kernel_outputs, dim=1).view(weighted_feats.shape[0],-1)  # (M, K, C, C_out/C) -> (M, C_out)
    ...
    def __repr__(self):
        if isinstance(self.kernel_size,int):
            ...
            kernel_size = sum(self.kernel_size)-len(self.kernel_size)+1
        format_string = self.__class__.__name__ + '('
        format_string += 'kernel_size: {}'.format(kernel_size)
        format_string += ', in_channels: {}'.format(self.in_channels)
        format_string += ', out_channels: {}'.format(self.out_channels)
        format_string += ', radius: {:g}'.format(self.radius)
        format_string += ', sigma: {:g}'.format(self.sigma)
