...
class EpochBasedTrainer:
    def __init__(self, cfg):
        ...
        self.step_steps = cfg.step_steps
        ...
        self.log_file:str = cfg.log_dir + '/' + cfg.runname + '_{}'
    ...
    def save_snapshot(self, filename, full=True):
        state_dict = {
            'epoch': self.epoch,
    ...
    def train_epoch(self):
        ...
        for iteration, data_dict in enumerate(self.train_loader):
            ...
            if (iteration + 1) % self.log_steps == 0:
                ...
                for k,v in self.summary_board.meter_dict.items():
                    loss_logs[k] = np.array(v._records)
                print("Logging into ", self.log_file+'.pkl')
                flog = open(self.log_file+'.pkl', 'wb')
                flog.write(pickle.dumps(loss_logs))
                flog.close()
        ...
        if isinstance(self.val_loader, list):
            for loader in self.val_loader:
                self.validate_epoch(loader)
        else: self.validate_epoch(self.val_loader)
    ...
    def validate_epoch(self, loader:DataLoader):
        self.set_eval_mode()
        ...
        for iteration, data_dict in enumerate(loader):
            ...
            print("[%d/%d]"%(iteration, len(loader)), end=' ')
            for key, value in result_dict.items():
                print(key, "%.4f"%float(value), end='; ')
            #torch.cuda.empty_cache()
            torch.cuda.synchronize()
        ...
        recall = summary_board.meter_dict['RR'].records
        trans_err = summary_board.meter_dict['TE'].records
        rot_err = summary_board.meter_dict['RE'].records
        recall, trans_err, rot_err = loader.dataset.benchmark(recall, trans_err, rot_err)
        message = "Validate Epoch %02d (%d iterations): "%(self.epoch, self.iteration)
        message += "ARR: %.4f; ATE: %.4f; ARE: %.4f\n"%(recall, trans_err, rot_err)
        for k,v in summary.items(): message += "%s:%.3f; "%(k,v)
        message += "\n"
        
        print(message)
        f = open(self.log_file + ".txt", 'a')
        f.write(message)
        f.close()
    ...
    def fit(self, resume_epoch=0, resume_log=None):
        if resume_epoch > 0:
            ...
            if resume_log is not None:
                self.log_file:str = self.log_dir + '/' + resume_log
                print('Logging from %s.'%self.log_file)
                f = open(self.log_file + '.pkl', 'rb')
                log: Dict[str, np.ndarray] = pickle.load(f)
                f.close(); data = dict()
                for k, v in log.items():
                    print(k, v.shape)
                    data[k] = v[:self.iteration].tolist()
