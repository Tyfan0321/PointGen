...
# Group A: compare different positional encoding for semi-dense feature aggregation
# no encoding (CASTv2)
# learnable rotary encoding
# absolute sinusoidal encoding
# absolute learnable encoding
# absolute learnable + sinusoidal encoding
    ...
    def forward(self,ref_points,src_points, ref_feats,src_feats, ref_points_c,src_points_c, ref_feats_c,src_feats_c):
        ...
        for i in range(self.blocks):
            ...
            ref_feats = self.consistency_aware_attentions[i](
                ref_feats, ref_feats, ref_token_indices, ref_points, ref_points
            )
            src_feats = self.consistency_aware_attentions[i](
                src_feats, src_feats, src_token_indices, src_points, src_points
            )
...
class SpotGuidedGeoTransformer_b2(nn.Module):
    def __init__(self, cfg):
        super(SpotGuidedGeoTransformer_b2, self).__init__()
        self.k = cfg.k            # num of neighbor points whose corresponding patches are candidate spots
        self.spots = cfg.spots    # num of neighbor points whose corresponding patches are selected as spots
        self.down_k = cfg.down_k  # num of nodes for down sampling and fusion with semi-dense points
        self.spot_k = cfg.spot_k  # num of points in a spot
    ...
    @torch.no_grad()
    def seeding(self, compatible_scores:torch.Tensor, confidence_scores:torch.Tensor):
        selection_scores = compatible_scores.ge(compatible_scores.max(-1, True)[0] * self.seed_threshold)  # (B, N)
        return selection_scores.nonzero(as_tuple=True)[1].unsqueeze(0)
